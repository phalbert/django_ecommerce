...
class ProductViewsTests(TestCase):
    def setUp(self):
        ...
        self.item = {
            ...
            'featured': False,
            'image': self.image}
        Product.objects.create(**self.item)

    def test_get_products_list(self):
        """
        If products exist, an appropriate message is displayed.
        """
    ...
    def test_featured_list(self):
        ...
        Product.objects.create(**featured)
    ...
    def test_product_search_succeds(self):
        """
        A ``GET`` to the products search

        """
        response = self.client.get(
            reverse('products:search'), data={'q': 'item'})
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.context['products'])
        self.assertTemplateUsed(response, 'products/search_results.html')

    def test_product_search_fails(self):
        """
        A ``GET`` to the products search

        """
        response = self.client.get(
            reverse('products:search'), data={'q': 'unknown'})
        self.assertEqual(response.status_code, 200)
